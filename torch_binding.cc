...
size_t _bytes(void* t) {
  return reinterpret_cast<at::Tensor*>(t)->numel() * 4;
}
...
float _float32Scalar(void* t) {
  return reinterpret_cast<at::Tensor*>(t)->item<float>();
}
...
void* _full(void* shape_ptr, int64_t shape_len, float val) {
  auto shape = at::IntArrayRef(reinterpret_cast<int64_t*>(shape_ptr), shape_len);
  auto t = at::full(shape, val);
  return new at::Tensor(t);
}

void* _identity(int64_t dim) {
  return new at::Tensor(at::eye(dim));
}
...
void* _matmul(void* tensor, void* other) {
  auto a = reinterpret_cast<at::Tensor*>(tensor);
  auto b = reinterpret_cast<at::Tensor*>(other);
...
void* _sum(void* tensor, void* axes_ptr, int64_t axes_len, bool keep_dims) {
  auto t = reinterpret_cast<at::Tensor*>(tensor);
  auto shape = at::IntArrayRef(reinterpret_cast<int64_t*>(axes_ptr), axes_len);
  return new at::Tensor(t->sum(shape, keep_dims));
}
